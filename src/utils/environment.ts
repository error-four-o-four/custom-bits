export const getSupportsTouch = (): boolean => window && 'ontouchstart' in window;
...
const option = ((supportsPassive) ? { passive: false } : false) as EventListenerOptions;
...
export const deactivateScrollWhileFocused = (target: HTMLElement, keys: string[]) => {
...
const deactivateScroll = (keys: string[]) => {
	...
	if (supportsTouch) window.addEventListener('touchmove', preventDefault, option);

	/**@todo wheel event for modal meh! */
	if (!supportsTouch && deactivatedKeys.length > 0) window.addEventListener('keydown', preventDefaultForScrollKeys, option);
}

const activateScroll = () => {
	if (!scrollDeactivated) return;

	if (supportsTouch) window.removeEventListener('touchmove', preventDefault, option);

	/**@todo wheel event for modal meh! */
	if (!supportsTouch && deactivatedKeys.length > 0) window.removeEventListener('keydown', preventDefaultForScrollKeys, option);
...
export const addTabindexAttributes = (target: HTMLElement, index: number = 0) => {
	if (!target.hasAttribute('role')) target.setAttribute('role', 'button');
	if (!target.hasAttribute('tabindex')) target.setAttribute('tabindex', `${index}`);
}
